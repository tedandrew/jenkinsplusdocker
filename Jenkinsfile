pipeline {
  ...
  parameters {
    booleanParam(name: 'dryRun', defaultValue: params.dryRun ?: true, description: '[WARNING] Enabling this option will abort the job at the first step. It is used only to import parameters when the Pipeline is running for the first time.')
  ...
  environment {
    SSH_DEPLOY = "docker run -d --restart unless-stopped ${params.publishPorts} ${params.volumesMount} ${params.containersNetwork} --name ${JOB_BASE_NAME} ${params.dockerRegistry}/${params.dockerhubRepo}:${params.imageTag}"
  }
  stages {
